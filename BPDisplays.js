...
{  
  ...
  constructor( captureReadyCallback, videoWidth, videoHeight, selectedCameraIndex=0, mirror=false, rotation=0 )
  {
...
{
  constructor( captureReadyCallback, videoWidth, videoHeight, selectedCameraIndex=0, mirror=false, rotation=0, maxNumHands=2 )
  {
    super( captureReadyCallback, videoWidth, videoHeight, selectedCameraIndex, mirror, rotation );
  ...
  **/
  onResults(results) 
  {
    ...
    if ( (this.mirroring || this.rotation != 0) && results && results.multiHandLandmarks )
    {
      ...
      for (const landmarks of results.multiHandLandmarks) 
        ...
        for (let i = 0; i < landmarks.length; i++) 
        {
          ...
          if (this.rotation != 0)
          {
            ...
            landmarks[i].x = (landmarks[i].x-.5)*cos(rotRadians) - landmarks[i].y*sin(rotRadians) + .5;              
            landmarks[i].y = (landmarks[i].y-.5)*cos(rotRadians) + originalX*sin(rotRadians) + .5;
          }
        }
    }
